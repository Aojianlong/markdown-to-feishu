...
一键将 Obsidian Markdown 文章（含本地图片）自动同步到飞书云文档。

> 💡 **适合人群**：使用 Obsidian 做笔记，想要快速分享到飞书的用户
...
```
请帮我安装 obsidian-to-feishu 这个 skill 的依赖
```
...
**Windows 用户注意**：路径可能是 `C:\Users\你的用户名\.claude\skills\obsidian-to-feishu`
</details>
...
```
请帮我配置 obsidian-to-feishu，我的 App ID 是 cli_xxxxxx，App Secret 是 xxxxxx
```
...
**手动配置方法**：
1. 找到文件 `~/.claude/skills/obsidian-to-feishu/config.example.json`
...
```
请用 /obsidian-to-feishu 同步我的文档 D:\obsidian\notes\我的文章.md
```
...
```
请检查 obsidian-to-feishu 的配置是否正确
```

### 查看文件路径

```
请帮我找到我的 Obsidian 笔记文件夹路径
```
...
**解决**：
```
请帮我更新 obsidian-to-feishu 到最新版本
```
...
2. **在 Claude Code 中发送**：
   ```
   请帮我测试 obsidian-to-feishu 的飞书连接
   ```
...
```
请帮我修改 obsidian-to-feishu 的默认图片宽度为 1000px
```
